...
install.packages(c("ade4", "BAT", "vegan", "tidyverse", "gawdis", "devtools", 
                   ...
                   "ggnewscale", "ggstar")) # rest omitted
...
# we create 3 groups: the multichoice, the continuous and the nominal variables
...
orig_diss <- BAT::originality(apply(comm, 2, sum), distance = distw.tot, abund=F)%>%as.vector()

# uniqueness 

uniq_diss <- BAT::uniqueness(apply(comm, 2, sum), distance = distw.tot)%>%as.vector()
